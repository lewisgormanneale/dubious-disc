...
export class TypeBackgroundGeneratorPipe implements PipeTransform {
  transform(types: PokemonType[]): string {
    if (!types || types.length === 0) {
      return AllPokemonTypeValues[0].darker_type_color;
    }
    ...
    if (numTypes === 1) {
      ...
      return AllPokemonTypeValues[typeNumber1].darker_type_color;
    }
    ...
    if (types.length === 2) {
      ...
      return `linear-gradient(to bottom right, ${AllPokemonTypeValues[typeNumber1ID].darker_type_color}, ${AllPokemonTypeValues[typeNumber2ID].darker_type_color})`;
    }
